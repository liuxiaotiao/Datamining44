...
def Iva(dataset):
    ...
    # print(first, second, third, forth)
    if (num1 != 0) & (num2!=0):
        I_res = -num1 * (np.log2(num1/(num1 + num2))) / (num1 + num2) - num2 * np.log2(num2/(num1 + num2)) / (num1 + num2)
    else:
        I_res = 0
    # print(-I_res)
    return I_res
...
def IVa(dataset):
    ...
    for i in range(len(c)):
        if (num_no[i] != 0) & (num_yes[i] != 0):
            iva -= (num_no[i] + num_yes[i]) *np.log2((num_no[i] + num_yes[i])/(sum(num_no) + sum(num_yes))/ (sum(num_no) + sum(num_yes)))
    # print(num_no, num_yes,entropy)
    return iva
    ...
    for i in range(num - 1):
        ...
        if isinstance(dataset[0][i], str):
            ...
            for item in dataset:
                m = [item[i], item[num - 1]]
                sI.append(m)
            ingain = Iplay - I_res(sI)
            if IVa(sI)==0:
                gainra = 1
            else:
                gainra = ingain/IVa(sI)
            entropy.append(gainra)
        else:
            b = []
            ...
            for n in b:
                ...
                searchration.append(Iva(c))
            for y in range(len(searchIres)):
                searchIres[y] += Iplay
            for x in range(len(searchIres)):
                if searchration[x]!=0:
                    searchration[x] = searchIres[x]/searchration[x]
                else:
                    searchration[x] =1
            entropy.append(max(searchration))
            #----
            s = 0
...
def build_tree(treenode):
    if treenode.stop():
        ...
        entro, spt, sph = gainratio(da, cli)
        max_entro = max(entro)
        record = -1
